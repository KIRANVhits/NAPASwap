...
const NFTs = ({ currentAccount }: Data) => {
  ...
  const handleSelectNFT = (nft: any) => {
    const isSelected = selectedNFTs.some(
      (selectedNFT: { token_id: any }) => selectedNFT.token_id === nft.token_id
    );
    if (isSelected) {
      setSelectedNFTs(
        selectedNFTs.filter(
          (selectedNFT: { token_id: any }) =>
  ...
  const handleNextClick = () => {
    ...
    if (queryParam && selectedNFTs.length > 0) {
      ...
      const senderNFTImage = new URLSearchParams(queryParam).get(
        "senderNFTImage"
      );
      const nftIds = selectedNFTs
        .map((nft: { token_id: string[] }) => nft.token_id[selectedNFTs])
        .join(",");
      const nftAddresses = selectedNFTs
        ...
        <button className="cnt-btn mt-2" onClick={handleNextClick}>
          <Link
            to={
              location.search
                ? `/CreateSwap?token_id=${
                    selectedNFTs.token_id
                  }&swapId=${new URLSearchParams(location.search).get(
                    "swapId"
                  )}&nftImage=${
                    selectedNFTs.image
                  }&senderNFTImage=${new URLSearchParams(location.search).get(
                    "senderNFTImage"
                  )}&nftAddress=${selectedNFTs.token_address}`
