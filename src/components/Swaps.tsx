...
const Swaps = ({ contract, signer, contractNft, currentAccount }: Data) => {
  ...
  const fetchAllSwaps = async () => {
    ...
    const swapDetails = await Promise.all(
      allSwaps.map(
        async (
          swap: { senderNFTId: { toString: () => any }; recipientNFTId: number[]; recipientNftAddress: string[] },
          index: number
        ) => {
          try {
            ...
            console.log(senderNFTURI, "senderURI");
            ...
            if (swap.recipientNFTId.length !== 0) {
              ...
              console.log(recipientNFTURI, "recipientURI");
            }
  ...
  const fetchSwapsByWalletAddress = async (currentAccount: string) => {
    ...
    const swapDetails = await Promise.all(
      userSwaps.map(
        async (swap: { senderNFTId: { toString: () => any }; recipientNFTId: number[] }, index: number) => {
          try {
            const senderNFTURI = await contractNft.tokenURI(
              swap.senderNFTId.toString()
            );
    ...
    <>
      ...
      <div className="d-flex justify-content-end mt-3">
        <button className="cnt-btn">
          <Link to="/NFTs" className="text-black text" >Create offer</Link>
        </button>
      </div>
        ...
        {swapDetails.map((swap, index) => (
          <div key={index} className="swap-item">
            <div className="card-body d-flex justify-content-between align-items-center">
              <div style={{ width: "200px", height: "200px", marginRight: "5px" }}>
                {/* <p className="card-text">ID: {swapIds[index]}</p> */}
                {swap.senderNFTImage && (<img className="nft-img rounded" src={swap.senderNFTImage} alt="Sender NFT" />)}
              </div>
              <div style={{ width: "200px", height: "200px", marginRight: "5px" }}>
                {swap.recipientNFTId.length !== 0 && swap.recipientNFTImage ? (
                  <img className="nft-img rounded" src={swap.recipientNFTImage} alt="Recipient NFT" />
                ) : (
                  <div className="blank-image-box rounded"><p className="waiting-text">Waiting for swap</p></div>
                )}
              </div>
              {swap.senderNFTId !== 0 && swap.recipientNFTId.length !== 0 ? (
                <div style={{ display: "flex", flexDirection: "column", margin: "14px" }}>
                  <button className="cmplt-btn text-white" onClick={(event) => handleConfirmSwap(event, swapIds[index])}>
                    Accept & Complete
                  </button>
                  <button className="rjct-btn text-white" onClick={(event) => handleCancleSwap(event, swapIds[index])}>
                    Reject
                  </button>
                  <button className="ccb-btn text-white" onClick={(event) => handleClaimBack(event, swapIds[index])}>
                    Cancel & Claim Back
                  </button>
                </div>
              ) : (
                swap.senderNFTId !== 0 && (
                  <button className="cs-btn mt-4">
                    <Link
                      to={`/NFTs?swapId=${swapIds[index]}&senderNFTImage=${encodeURIComponent(swap.senderNFTImage)}`}
                      className="text-white text" onClick={() => console.log(swapIds[index])}
                    >
                      Create Swap
                    </Link>
                  </button>
